...
# Full integrated holdings + trading plan dashboard
# Copy this whole file into your Streamlit pages folder (replace existing)
...
# ------------------ Configuration ------------------
st.set_page_config(layout="wide", page_title="GM Trade Dashboard — Holdings + Trading Plan")
st.title("📊 Trading Dashboard — Definedge (Risk Managed — Integrated)")
...
# ------------------ Helpers ------------------
def safe_float(x):
    """Convert possible numeric-like input to float or None"""
    if x is None:
        return None
    try:
...
def safe_num_zero(x):
    """Convert to float, return 0.0 on failure or None"""
    v = safe_float(x)
...
# Fetch holdings (your original logic)
try:
...
# pnl calculations (robust)
# prefer API-provided realized pnl if available in raw
...
# ensure numeric initial_risk & open_risk exist
for col in ['initial_risk', 'open_risk', 'realized_if_tsl_hit']:
...
def calc_stops_targets(row):
    avg = safe_num_zero(row.get('avg_buy_price'))
    qty = int(row.get('quantity') or 0)
    ltp = safe_num_zero(row.get('ltp'))
    if qty == 0 or avg == 0:
...
# Portfolio KPIs (safe numeric conversions)
total_invested = safe_num_zero(df['invested_value'].sum())
total_current = safe_num_zero(df['current_value'].sum())
total_overall_pnl = safe_num_zero(df['overall_pnl'].sum())
missing_prev_count = int(df['prev_close'].isna().sum())
total_today_pnl = safe_num_zero(df['today_pnL'].fillna(0.0).sum())
total_initial_risk = safe_num_zero(df['initial_risk'].sum())
total_open_risk = safe_num_zero(df['open_risk'].sum())
total_realized_if_all_tsl = safe_num_zero(df['realized_if_tsl_hit'].sum())

# Display KPIs (guard formatting)
st.subheader('💰 Overall Summary')
k1, k2, k3, k4, k5 = st.columns(5)
try:
    k1.metric('Total Invested', f'₹{total_invested:,.2f}')
except Exception:
    k1.metric('Total Invested', f'₹{total_invested}')
try:
    k2.metric('Total Current', f'₹{total_current:,.2f}')
except Exception:
    k2.metric('Total Current', f'₹{total_current}')
try:
    k3.metric('Unrealized PnL', f'₹{total_overall_pnl:,.2f}')
except Exception:
    k3.metric('Unrealized PnL', f'₹{total_overall_pnl}')
# Today pnl metric (with delta shown)
...
try:
    ...
    k5.metric('Open Risk (TSL)', f'₹{total_open_risk}')
...
# optional trade-log upload for accurate win rate calculation
st.info("Optional: Upload a trade-log CSV (columns: symbol, entry_date, exit_date, pnl) to compute actual win-rate and avg holding days.")
...
# All open positions SL (using initial_risk)
total_open_positions = df.shape[0]
total_open_risk = safe_num_zero(df['initial_risk'].sum())
...
max_drawdown_alert_pct = st.sidebar.number_input('Set drawdown % for alerts', min_value=1.0, max_value=50.0, value=5.0)
col3.metric("Allowed max drawdown (user)", f"₹{capital * (max_drawdown_alert_pct/100):,.0f}")
...
if realized_available:
    ...
    elif wins_count >= 1 and wins_count < 10:
...
st.subheader("📈 Capital Growth Projection (Expectation-based)")
...
proj_df = pd.DataFrame(capital_series) if capital_series else pd.DataFrame(columns=['month','capital','expected_gain'])
if not proj_df.empty:
    ...
    st.info("Projection table empty (no months).")
...
plan_summary = {
    ...
    "Trades_needed_for_target": (math.ceil(trades_needed) if trades_needed != math.inf else None),
    "Expected_days_to_target": (int(expected_total_days) if expected_total_days != math.inf else None),
