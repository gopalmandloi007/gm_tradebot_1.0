...
def get_api_session_key(client):
    try:
        return client._api_session_key  # <-- same property we used in order placement
    except Exception:
        return None
...
if master_file is not None:
    ...
    st.write(f"✅ Filtered symbols: {len(master_df)}")
            ...
            for _, row in subset_df.iterrows():
                ...
                try:
                    ...
                    if resp.status_code == 200 and resp.text.strip():
                        ...
                        st.warning(f"{sym}: API returned empty or error {resp.status_code}")
